...
function login(req, res) {
    userServices.login(req.body).then(
        (user) => {
            user ? res.json(user) : res.status(statusNames.BadRequest).json({ message: "Incorrect Email or password" })
        },
        (error) => { res.status(statusNames.InternalServerError).json({message: error.message}) }
    )
}