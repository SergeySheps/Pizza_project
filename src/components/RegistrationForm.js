...
class RegisterForm extends Component {
    ...
    handlesubmit = (e) => {
        ...
        if (user.firstName && user.secondName && user.email && user.password && user.confirmedPassword) {
            ...
            dispatch(userActions.register(userWithoutConfirm));
        }
    }

    render() {
        const { registering, alert, registered, dispatch } = this.props;
        const { user, submitted, isSamePasswords} = this.state;
        if (registered) {
            dispatch(userActions.registerComplited());
        ...
        return (
            ...
            <div className="col-md-6 col-md-offset-3">
                ...
                <form name="form" onSubmit={this.handlesubmit}>
                    ...
                    <div className={'form-group' + (submitted && (!user.confirmedPassword || !isSamePasswords) ? ' has-error' : '')}>
                    ...
                    <div className="form-group">
                        {!registering && <button type="submit" className="btn btn-primary">Register</button>}
                        {registering &&
                            <img alt="loading..." 
