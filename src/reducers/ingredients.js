...
export function ingredients(state = [], action) {
    switch (action.type) {
        case pizzaConstants.PIZZA_ADD_INGREDIENT: {
        ...
        case pizzaConstants.PIZZA_UPDATE_INGREDIENTS: {
        ...
        case pizzaConstants.PIZZA_REDUCE_INGREDIENT: {
            return state.map(ingredient =>
                (ingredient.name === action.ingredient.name)
                    ? { ...ingredient, amount: ingredient.amount > 0?--ingredient.amount:ingredient.amount, price: ingredient.basePrice * ingredient.amount }
                    : ingredient
            )
        }
